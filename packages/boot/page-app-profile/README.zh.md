...
- **启动恢复绝不猜测** —— 与 journal token 匹配的死 `manager` 锁由恰好一个恢复者改名隔离（失败者直接失败退出）；无 journal 的死 `manager` 锁可安全移除，因为任何变更都不得先于 journal 发布；其余一切状态——活进程、token 不匹配、payload 不可读、活性不确定，或任何死 `plugin-cli` 锁——都失败闭合交由人工修复。
...
- **隔离与 claim 文件被保留** —— 恢复的死锁以 `<token>.quarantine` 名字留存，恢复 claim 文件也保留在本地作为取证证据；尚无自动清理。
