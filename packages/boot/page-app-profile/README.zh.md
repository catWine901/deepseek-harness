...
- **启动恢复绝不猜测** —— 与 journal token 匹配的死 `manager` 锁由恰好一个恢复者改名隔离：恢复 claim 构成按 token 追加的 `wx` 创建代际链，存活或不可读的链尾失败闭合，可证明已死的链尾被下一代取代，失败者直接失败退出。无 journal 的死 `manager` 锁可安全移除，因为任何变更都不得先于 journal 发布；其余一切状态——活进程、token 不匹配、payload 不可读、活性不确定，或任何死 `plugin-cli` 锁——都失败闭合交由人工修复。
