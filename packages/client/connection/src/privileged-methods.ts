 ...
 * browser cannot.
 *
 * The page-app manager's seven mutations are the workspace-apps transport
 * additions: they install, enable, hide, reorder, uninstall, acknowledge, and
 * recover the managed workspace roots the profile runtime composes, so an
 * anonymous LAN caller must not reach them through either dispatch path — the
 * Typert Gateway, which forms endpoints as `${namespace}/${method}` (hence
 * the slash spelling), or the legacy API Proxy, whose names stay dotted. One
 * transport's spelling is never normalized into the other's and no prefix
 * matching exists.
 *
