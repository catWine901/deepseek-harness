...
`PageAppManager` extends the Typert Remote service `pageAppManager`. Every mutation runs inside the shared profile mutation lock and writes a prepared journal plus private before-state backups before any owned file changes; a failed transaction rolls back and retains the journal as `recovery-required`, and the operator `recover()` Remote resolves it. The generated Host and Client Remote artifacts are exposed by `./typert` and `./remote`.
