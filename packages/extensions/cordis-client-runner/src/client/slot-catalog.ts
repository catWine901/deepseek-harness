...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  {
    ...
    hookContext: '',
    slotInject: '',
    declaredBy: 'an entry in \'page-app.shell.builtin\' (client-ui-layout), so it exists while that entry is mounted',
    occupants: [
  ...
  {
    key: 'page-app.shell.builtin',
    kind: 'single',
    scope: 'root',
    summary: 'The built-in Original DSH seat: the one permanent system surface that the shell mounts unconditionally and hides (never unmounts) while a managed surface is active.',
    doc: 'The built-in Original DSH seat: the one permanent system surface that\nthe shell mounts unconditionally and hides (never unmounts) while a\nmanaged surface is active. OCCUPIED by ui-layout\'s AppFrame.',
    registerOptions: [],
    ownerProps: [
    ...
    hookContext: '',
    slotInject: '',
    declaredBy: 'an entry in \'root\' (client-ui-page-app-manager), so it exists while that entry is mounted',
    occupants: [
      'client-ui-layout AppFrame',
    ],
    replaceRisk: 'shadows-shipped-ui',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'page-app.shell.builtin\', () => ctx.slots.register(\n      { name: \'page-app.shell.builtin\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-page-app-manager/src/client/contracts.ts:104',
  },
  {
    key: 'page-app.shell.surface',
    kind: 'keyed',
    scope: 'root',
    summary: 'One full-page managed surface per keyed page id.',
    doc: 'One full-page managed surface per keyed page id. OCCUPIED by managed\npackages after runtime activation; the closed authorization projection\n(spec §7) keeps unrelated contributions invisible.',
    registerOptions: [
      {
        name: 'key',
        requirement: 'required',
    ...
    occupants: [],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'page-app.shell.surface\', () => ctx.slots.register(\n      { name: \'page-app.shell.surface\', key: \'<one key the owner dispatches>\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-page-app-manager/src/client/contracts.ts:110',
  },
  {
    key: 'root',
    kind: 'single',
    ...
    occupants: [
      'client-ui-layout AppFrame',
      'client-ui-page-app-manager PageAppShell',
    ],
    replaceRisk: 'shadows-shipped-ui',
  ...
  },
  {
    ...
    occupants: [
      'client-ui-page-app-manager PageAppSettingsTab id \'workspace-apps\'',
