...
[Workspace Apps](subsystems/workspace-apps.md) is an optional out-of-tree manager installed per Profile rather than a permanent Web bundle layer. It provides the versioned Workbench Runtime that managed Feature wrappers depend on; without it, those wrappers stay inactive and `ui-layout` renders Native DSH through its root fallback.
