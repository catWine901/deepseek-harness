...
- Every npm package is `@deepseek-ai/dsh-<name>`; vendored packages are rescoped ([mapping](docs/rescope.md)) and `private: true`. Ordinary harness packages list `@deepseek-ai/cordis` as peerDependency (+ dev). Valid `dsh.workspace.schemaVersion: 1` Workspace Features are Cordis-free and consume only the Workbench Contract.
