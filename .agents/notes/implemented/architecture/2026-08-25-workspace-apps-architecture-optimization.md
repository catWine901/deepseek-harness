...
- **D3 — Cordis adapter:** manager product code delegates Cordis, Include, Loader, hashing, fiber projection, and wrapper mounting through `src/adapter.ts`; runtime Cordis imports elsewhere are rejected.
...
- **D10 — packaging:** the published tarball is scanned for forbidden members and absolute local paths, contains no `workspace:` specifiers, and passes a fresh-Profile install/disable/re-enable/uninstall chain. CI requires the active pnpm version to equal `packageManager`.
...
- **D12 — legacy removal:** a legacy path is deleted only after its replacement's named proof passes. The runtime has one Profile watcher, one wrapper form for managed roots, one Cordis adapter, and no permanent manager rows in `dsh-web-app`.
...
- The packed install-chain smoke validates extraction, final tarball bytes, a fresh Profile, manager disable with Native DSH, re-enable, and uninstall before the shipped rows are absent.
...
- Framework changes concentrate in the adapter and contract implementation; official Feature sources use the versioned workbench API instead of Cordis.
