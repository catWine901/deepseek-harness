...
- **D3 — Cordis 适配器：** 管理器产品代码通过 `src/adapter.ts` 委托 Cordis、Include、Loader、哈希、fiber 投影与包装层挂载；其他位置的 Cordis 运行时导入会被拒绝。
...
- **D10 — 打包：** 已发布 tarball 会扫描禁止成员与绝对本地路径，不含 `workspace:` 说明符，并通过全新 Profile 的安装/停用/重新启用/卸载链。CI 要求活动 pnpm 版本与 `packageManager` 相等。
- **D11 — CLI 共存：** 通用插件变更共用 Profile 锁。声明 `dsh.workspace` 的包被分类为工作区应用，`dsh plugin` 不会把它们提升到 `dsh.profile.bundles`。
- **D12 — 旧路径移除：** 旧路径仅在替代机制的具名证明通过后删除。运行时只有一个 Profile watcher、一个托管根包装层形态、一个 Cordis 适配器，且 `dsh-web-app` 中没有常驻管理器行。
...
- 打包安装链 smoke 在已交付行缺席前验证提取、最终 tarball 字节、全新 Profile、停用管理器时的 Native DSH、重新启用与卸载。
...
- 框架变化集中在适配器与约定实现中；官方功能源码使用版本化 Workbench API，而不使用 Cordis。
